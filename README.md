...
| `wyntechmapv1.html` | The interactive company map (MapLibre GL JS + MapTiler). Self-contained, with its own data built in. Embedded inside `index.html` and also works on its own. |
...
**Run it locally:** the map's own data is built directly into the file, so it works the moment you open it, no server needed for that part. The map tiles themselves (MapTiler) and the growth charts (Chart.js) still need a real internet connection either way, since those load from external services regardless of how you're viewing the page.
...
## Map setup (MapTiler key)

The map runs on [MapLibre GL JS](https://maplibre.org/) (an open-source map renderer) with map tiles from [MapTiler](https://www.maptiler.com/). It needs an API key to render, set in `wyntechmapv1.html` as `const MAPTILER_KEY = '...'` near the top of the script.

**If you're forking this project to deploy on your own domain, get your own free key:**

1. Create a free account at [maptiler.com](https://www.maptiler.com) (the free tier covers far more usage than this project needs).
2. Go to **API keys** in your MapTiler dashboard and copy your default key, or create a new one.
3. In `wyntechmapv1.html`, find `const MAPTILER_KEY='...'` and swap in your key.
4. On that key's settings page, add your domain under **Allowed HTTP origins**, so usage stays tied to your deployment rather than being usable from anywhere.
...
- [MapLibre GL JS](https://maplibre.org/) `v3.6.2` with [MapTiler](https://www.maptiler.com/) tiles — the interactive map
- [Chart.js](https://www.chartjs.org/) `v4.4.0` and the annotation plugin `v3.0.1` — the growth charts
- Plain HTML, CSS, and JavaScript — no framework, no build step

MapTiler and Chart.js load from their public CDNs, so the map and charts need an internet connection to render.
